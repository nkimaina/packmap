...
module.exports = async function packmap(opts) {
  ...
  const excludePackages = await getExcludePackageFile();
  const importMap = await traversePackage(
    packageJson,
    { imports: {} },
    excludePackages
  );
  ...
  async function getExcludePackageFile() {
    if (opts.excludePackages) {
      const excludePackagesPath = path.resolve(cwd, opts.excludePackages);
      if (await fs.pathExists(excludePackagesPath)) {
        log(`Excluding packages as per "${opts.excludePackages}"`);
        return JSON.parse(fs.readFileSync(excludePackagesPath));
      } else {
        throw new Error(
          `Unable to find exclude-packages file at "${excludePackagesPath}"`
        );
      }
    }
  }
  ...
  function traversePackage(
    ...
    return Promise.all(
      dependencies.map(dependency => {
        ...
        if (excludePackages && excludePackages[dependency]) {
          if (opts.verbose) {
            log(`excluding package ${dependency}`);
          }
          return;
        }
        if (opts.verbose) {
